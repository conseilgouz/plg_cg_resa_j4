<?php
/**
* CG Resa Plugin  - Joomla 4.x/5.x plugin
...
class plgcontentcgresaInstallerScript
{
    private $min_joomla_version      = '4.0.0';
    private $min_php_version         = '8.0';
    private $name                    = 'Plugin CG Resa';
    ...
    private function postinstall_cleanup()
    {
        ...
        $query = $db->getQuery(true)
            ...
            ->where($db->qn('element') . ' = ' . $db->quote('cg_resa'));
        $db->setQuery($query);
        $db->execute();
        // CG Resa is now on Github
        $query = $db->getQuery(true)
            ->delete('#__update_sites')
            ->where($db->quoteName('location') . ' like "%conseilgouz.com/updates/plg_cg_resa%"');
        $db->setQuery($query);
        $db->execute();
